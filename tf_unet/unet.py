...
def create_conv_net(x, keep_prob, channels, n_class, chanel_root = 4, field_of_view = 3, max_pool_size = 2):
    ...
    with tf.name_scope("convolution_1"):
        ...
        tf.histogram_summary("convolution_1" + '/activations', h_conv1)
...
class Unet(object):
    ...
    def __init__(self, nx, ny, channels=3, n_class=2):
        ...
        self.y = tf.placeholder("float", shape=[None, nx-184,ny-184, n_class])
...
#         self.predicter = pixel_wise_softmax(logits)
    ...
    def predict(self, model_path, x_test):
        ...
        with tf.Session() as sess:
            ...
            y_dummy = np.empty((x_test.shape[0], self.nx-184, self.ny-184, self.n_class))
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False):
        ...
        with tf.Session() as sess:
            ...
            self.store_prediction(sess, data_provider, "start")
            ...
            for epoch in range(epochs):
                ...
                save_path = self.net.save(sess, os.path.join(output_path, "model.cpkt"))
